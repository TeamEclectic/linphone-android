...
\subsection{getContactList} \label{FR-invite}
\paragraph{Summary:} This functionality retrieves the user creating a new groups' contact list.
\paragraph{Rationale:} This will be used when a user tries to add a contact to a group. The user's contact list will be retrieved and the member the contact is trying to add will be checked to see if they are a contact in the users list and if that contact has the Linphone service.
\paragraph{Requirements:} When this functionality is activated the users contact list should be retrieved.
