...
\paragraph{Summary:} A user will be able to create a new group.
...
\paragraph{Summary:} An Invite will be sent when a group is created with new members or when a new member is added to the group.
...
\paragraph{Summary:} A message object sent on a group will be received by every member subscribed to a group.
...
%If the user chooses to change the profile picture, the user may be afforded the option to remove the profile picture (replacing it with a default system image) or allow the user to choose a new picture by invoking the file manager in order to choose a picture from local storage. The updated profile picture should be visible to all users within the group after the change has taken place.
